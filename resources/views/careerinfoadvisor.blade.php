...
@endif
        <h4 class="card-title">{{__('Edit profile information')}}</h4>
         ...
         @endphp
{!! Form::model($userprofile , ['route' => ['careerinfo', $userprofile->id], 'method'=>'PATCH','enctype'=>'multipart/form-data']) !!}
                ...
                <div class="form-group">
                    ...
                    {!! Form::text('amount', null, array('placeholder' => __('Price'),'class' => 'form-control')) !!}
                </div>

               <div class="form-group">
                    <label>  نوع الخدمة</label>
                    <select class="form-control" name="service_id">
                        @foreach($Servicesx as $service)
                        <option value="{{$service->id}}"{{($userprofile->service_id == $service->id) ? 'selected': ''}} >{{$service->title}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group">
                    <label>  {{__('Payment gateway email pay pal')}} </label>
                    {!! Form::text('emailpay', null, array('placeholder' => __('example@ex.com'),'class' => 'form-control')) !!}
                </div>
            <div class="form-group">
              <label> {{__('Biography')}}</label>
              {!! Form::textarea('description', null, array('placeholder' => __('My Bio'),'class' => 'form-control','cols'=>'40','rows'=>'5')) !!}
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <label>  {{__('Link')}} </label>
                {!! Form::text('link', null, array('placeholder' => 'https://link.com','class' => 'form-control')) !!}
            </div>
                <div class="form-group">
                  <p>يرجى رفع السيرة الذاتية / الشهادة الجامعية</p>
            ...
            <br>
             @isset($userprofile->files)
            @foreach ($userprofile->files as $key => $item)
          ...
          <a href="{{asset('/storage/'.$item)}}" target="_blank">
           ...
           <a href="{{ route('contacts.files.destroy', [$userprofile->id, $key])}}" class="btn-xs btn-danger">Delete</a>
           ...
           <a href="{{ route('contacts.files.destroy', [$userprofile->id, $key])}}" class="btn-xs btn-danger">Delete</a>
            ...
            <a href="{{asset('/storage/'.$item)}}">file[{{$key}} - {{ $userprofile->id}}]</a>
            <a href="{{ route('contacts.files.destroy', [$userprofile->id, $key])}}" class="btn-xs btn-danger">Delete</a>
            <br>
             @endif
