    ...
    <div class="card mb-4">
      <div class="card-body">
        ...
        <div class="row">
            ...
            <div class="col-sm-9">
             <a href="{{route('marksreadnotifiyfromadmin.showuser',$item->id)}}"> <p class="text-muted mb-0">    
            @php
              $t=json_decode($item->data);
            @endphp
            {{$t->title}}
            </p></a>
             <a href="{{route('marksreadnotifiyfromadmin.showuser',$item->id)}}"> <p class="text-muted mb-0"> {{$item->created_at}} </p></a>
            </div>
          </div>
          <hr>  
          @empty
