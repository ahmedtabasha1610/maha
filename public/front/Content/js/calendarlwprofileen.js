...
var datetimepickerFactory = function ($) {
	...
	var dateFormatters = {
		moment: {
			default_options: {
				format: 'YYYY/MM/DD HH:mm AM/PM : HH2:mm2 AM/PM ',
				formatDate: 'YYYY/MM/DD',
				formatTime: 'HH:mm AM/PM : HH2:mm2 AM/PM ',
			},
			formatter: {
	...
	$.fn.datetimepicker = function (opt, opt2) {
		...
		createDateTimePicker = function (input) {
			var datetimepicker = $('<div class="xdsoft_datetimepicker xdsoft_noselect"></div>'),
				...
				applyButton = $('<button type="button" class="xdsoft_save_selected blue-gradient-button">Continue</button>'),
				...
				.on('xchange.xdsoft', function (event) {
					...
					xchangeTimer = setTimeout(function () {
						...
						if (options.minTime !== false) {
							var t = _xdsoft_datetime.strtotime(options.minTime); minTimeMinutesOfDay = 60 * t.getHours() + t.getMinutes();
						}
						var maxTimeMinutesOfDay = 24 * 60;
						if (options.maxTime !== false) {
							var t = _xdsoft_datetime.strtotime(options.maxTime); maxTimeMinutesOfDay = 60 * t.getHours() + t.getMinutes();
						}
						...
						if (!options.allowTimes || !$.isArray(options.allowTimes) || !options.allowTimes.length) {
							for (i = 0, j = 0; i < (options.hours12 ? 12 : 24); i += 1) {
								for (j = 0; j < 60; j += options.step) {
									var currentMinutesOfDay = i * 60 + j; if (currentMinutesOfDay < minTimeMinutesOfDay) continue; if (currentMinutesOfDay >= maxTimeMinutesOfDay) continue; h = (i < 10 ? '0' : '') + i;
									m = (j < 10 ? '0' : '') + j; line_time(h, m);
								}
							}
						} else {
